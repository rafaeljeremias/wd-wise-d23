...
const
  ...
  TOcorrenciaSantanderString : array[0..113] of array [0..3] of String =
...
type
  ...
  TLoteListSantander = class;
  TLoteSantander = class;
  TSegmentoA = class;
  TSegmentoJ = class;
  TSegmentoN = class;
  TSegmentoO = class;

  TArquivoSantander = class(TInterfacedObject, IArquivo)
  private
    fARQUIVO : String;
    ...
    fHORA_GERACAO : TTime;
    fDENSIDADE : integer;
    fLoteListSantander : TLoteListSantander;
    fLinhas : integer;
  ...
  public
    ...
    property LOTELISTSANTANDER : TLoteListSantander read fLoteListSantander write fLoteListSantander;
    ...
    function retornaLote(pagamento: TPagamentoAlPagtoE): TLoteSantander;
    function getLote(codigo : integer) : TLoteSantander;
    ...
    procedure atualizarDadosLote(var pagamento: TPagamentoAlPagtoE; lote : TLoteSantander);
  ...
  private
    ...
    fLayoutSantander: TArquivoSantander;
  public
    constructor Create(AOwner: TArquivoSantander);
    ...
    property LAYOUTSANTANDER: TArquivoSantander read fLayoutSantander;
  ...
  private
    ...
    fOCORRENCIAS : String;
    fLoteSantander: TLoteSantander;

  public
    constructor Create(AOwner: TLoteSantander); virtual; /// Create
    ...
    property OCORRENCIAS : String read fOCORRENCIAS write fOCORRENCIAS;
    property LOTESANTANDER: TLoteSantander read fLoteSantander;
  ...
  public
    function New(AOwner: TLoteSantander): TSegmentoA;
  ...
  public
    function New(AOwner: TLoteSantander): TSegmentoJ;
    function LocalizaRegistro(Registro: integer): boolean;
    function ValorPagamentos : double;
  ...
  public
    function New(AOwner: TLoteSantander): TSegmentoJ52;
  ...
  public
    function New(AOwner: TLoteSantander): TSegmentoN;
  ...
  public
    function New(AOwner: TLoteSantander): TSegmentoO;
    function ValorPagamentos : double;
    property Items[Index: Integer]: TSegmentoO read GetItem write SetItem;
  end;

  TLoteListSantander = class(TObjectList)
  private
    function GetItem(Index: Integer): TLoteSantander;
    procedure SetItem(Index: Integer; const Value: TLoteSantander);
  public
    function New(AOwner: TArquivoSantander): TLoteSantander;
    function LocalizaRegistro(Codigo: integer): boolean;
    property Items[Index: Integer]: TLoteSantander read GetItem write SetItem;
  end;
...
function TSegmentoAList.New(AOwner: TLoteSantander): TSegmentoA;
begin
...
constructor TSegmentoA.Create(AOwner: TLoteSantander);
begin
  fLoteSantander := AOwner;
end;
...
begin
  if (fLoteSantander.fTP_PAGAMENTO = tpFornecedores)and(fLoteSantander.fFORMA_PAGAMENTO = fpCreditoContaCorrenteBanco) then
    result := fFINALIDADE_DETALHE_OUTRO
  else
    if(fLoteSantander.fIDENTIFICA_EXTRATO) and
      (fLoteSantander.fTP_PAGAMENTO in [tpSalarios, tpSalariosPublico])and
      (fLoteSantander.fFORMA_PAGAMENTO in [fpCreditoContaCorrenteBanco, fpCartaoSalario]) then
...
begin
  if fLoteSantander.fFORMA_PAGAMENTO = fpDocC then
    result := FinalidadeDocToStr(fFINALIDADE_DOC)
  else
    if (fLoteSantander.fTP_PAGAMENTO = tpSalarios) and (fLoteSantander.FORMA_PAGAMENTO = fpCreditoContaCorrenteBanco) then
...
begin
  if (fLoteSantander.FORMA_PAGAMENTO in [fpCreditoContaCorrenteBanco, fpChequePagamento, fpDocC,
      fpCreditoContaPoupancaBanco, fpCreditoContaCorrenteTitular, fpDocD, fpTedOutroTitular,
      fpTedMesmoTitular]) then
    result := LayoutToStr(lySantander) +
              formataNumero(fLoteSantander.fCODIGO, 4) +                                //4
              '3' +                                                                     //8
              formataNumero(fREGISTRO, 5) +                                             //9
              'A' +                                                                     //14
              formataNumero(TipoMovToStr(fTP_MOVIMENTO),3) +                            //15
              ifThen(fLoteSantander.FORMA_PAGAMENTO = fpDocC, CamaraCentralizadoraTipoToStr(cctDOC), // 18
                ifThen((fLoteSantander.FORMA_PAGAMENTO = fpTedOutroTitular)and(fISPB = ''), CamaraCentralizadoraTipoToStr(cctTED),
                   ifThen((fLoteSantander.FORMA_PAGAMENTO = fpTedOutroTitular)and(fISPB <> ''), CamaraCentralizadoraTipoToStr(cctTED_ISPB), '000'))) +
              ...
              formataAlfa(fLoteSantander.LayoutSantander.TipoMoedaToStr(fTP_MOEDA),3) + //102
              DupeString('0',15) +                                                      //105
              ...
              quebra_linha
 { else
    if (fLoteSantander.FORMA_PAGAMENTO = fpNotaFiscal) then
    begin
      result := LayoutToStr(lySantander) +
        formataNumero(fLoteSantander.fCODIGO, 4) +
        '3' +
        ...
        formataAlfa(fLoteSantander.fLayoutSantander.TipoMoedaToStr(fTP_MOEDA),3) +
        DupeString('0',15) +
...
procedure TLoteSantander.addSegmento(const pagamento: TPagamentoAlPagtoE);
begin
...
procedure TLoteSantander.AddSegmentoA(const pagamento: TPagamentoAlPagtoE);
begin
...
procedure TLoteSantander.AddSegmentoJ(const pagamento: TPagamentoAlPagtoE);
begin
...
procedure TLoteSantander.AddSegmentoJ52(const pagamento: TPagamentoAlPagtoE);
begin
...
procedure TLoteSantander.AddSegmentoN(const pagamento: TPagamentoAlPagtoE);
begin
...
procedure TLoteSantander.AddSegmentoO(const pagamento: TPagamentoAlPagtoE);
begin
...
constructor TLoteSantander.Create(AOwner: TArquivoSantander);
begin
  fSEGMENTO_SEQ := 0;
  ...
  fLayoutSantander := AOwner;
end;

destructor TLoteSantander.Destroy;
begin
...
function TLoteSantander.getLinhaLayout: String;
begin
  result := LayoutToStr(lySantander) +
            ...
            formataAlfa(fLayoutSantander.CODIGO_CONVENIO, 20) +                                       //33
...
function TLoteSantander.getLinhaTrailer: String;
begin
...
function TLoteSantander.incSegmentoSeq(incremento: integer): integer;
begin
...
function TLoteListSantander.GetItem(Index: Integer): TLoteSantander;
begin
  Result :=  TLoteSantander(Inherited Items[Index]);
end;

function TLoteListSantander.LocalizaRegistro(Codigo: integer): boolean;
var
  intFor: integer;
begin
   Result := false;
...
function TLoteListSantander.New(AOwner: TArquivoSantander): TLoteSantander;
begin
  Result := TLoteSantander.Create(AOwner);
  Add(Result);
end;

procedure TLoteListSantander.SetItem(Index: Integer; const Value: TLoteSantander);
begin
  Put(Index, Value);
end;
...
procedure TArquivoSantander.addLinha(linha: String);
var
  tipo : String;
begin
  tipo := copy(linha, 8, 1);
...
procedure TArquivoSantander.addLinhaArquivo(linha: String);
var
  ok : boolean;
begin
...
procedure TArquivoSantander.addLinhaLote(linha: String);
var
  ok : boolean;
begin
  with fLoteListSantander.New(self) do
  begin
...
procedure TArquivoSantander.addLinhaSegmento(linha: String);
var
  segmento : string;
begin
...
procedure TArquivoSantander.addLinhaSegmentoA(linha: String);
var
  lote : TLoteSantander;
...
procedure TArquivoSantander.addLinhaSegmentoJ(linha: String);
var
  lote : TLoteSantander;
...
procedure TArquivoSantander.addLinhaSegmentoN(linha: String);
var
  lote : TLoteSantander;
...
procedure TArquivoSantander.addLinhaSegmentoO(linha: String);
var
  lote : TLoteSantander;
...
procedure TArquivoSantander.addPagamento(pagamento: TPagamentoAlPagtoE; TP_INSCRICAO : TTipoInsALPagtoE;
                                    INSCRICAOMATRIZ, fAGENCIA, fCONTA, fDAC_AGENCIA_CONTA, fNOME_EMPRESA, pCODIGO_CONVENIO: String;
                                    pCODIGO_REMESSA: Integer; pDT_GERACAO: TDate; pHR_GERACAO: TTime; pDIGITO_AGENCIA, pDIGITO_CONTA: String);
var
  lote : TLoteSantander;
...
procedure TArquivoSantander.atualizarDadosLote(
  var pagamento: TPagamentoAlPagtoE; lote: TLoteSantander);
begin
...
function TArquivoSantander.CarregarArquivo(nome_arquivo: string): boolean;
var
  layout : TextFile;
  linha : String;
...
procedure TArquivoSantander.clearPagamento;
begin
  fLoteListSantander.Clear;
  fRetorno.Clear;
end;

function TArquivoSantander.contaRetorno: integer;
begin
  result := fRetorno.Count;
end;

constructor TArquivoSantander.Create;
begin
  fLoteListSantander := TLoteListSantander.Create;
  fRetorno := TPagamentoList.Create;
end;

destructor TArquivoSantander.Destroy;
begin
  fLoteListSantander.Free;
  fRetorno.Free;
end;

function TArquivoSantander.GerarArquivo(nome_arquivo:string) : boolean;
var
  iLote, iSeg : integer;
  layout : TextFile;
...
begin
  try
    ...
    //lotes
    for iLote := 0 to fLoteListSantander.Count - 1 do
    begin
      write(layout, fLoteListSantander.Items[iLote].getLinhaLayout);
      inc(fLinhas);

      //segmento A
      for iSeg := 0 to fLoteListSantander.Items[iLote].fLISTSEGMENTOA.Count - 1 do
        write(layout, fLoteListSantander.Items[iLote].LISTSEGMENTOA.Items[iSeg].getLinhaLayout);

      //segmento J e J52
      for iSeg := 0 to fLoteListSantander.Items[iLote].fLISTSEGMENTOJ.Count - 1 do
      begin
        write(layout, fLoteListSantander.Items[iLote].LISTSEGMENTOJ.Items[iSeg].getLinhaLayout);
        write(layout, fLoteListSantander.Items[iLote].LISTSEGMENTOJ52.Items[iSeg].getLinhaLayout);
      end;

      //segmento N
     for iSeg := 0 to fLoteListSantander.Items[iLote].fLISTSEGMENTON.Count - 1 do
        write(layout, fLoteListSantander.Items[iLote].LISTSEGMENTON.Items[iSeg].getLinhaLayout);

      //segmento O
      for iSeg := 0 to fLoteListSantander.Items[iLote].fLISTSEGMENTOO.Count - 1 do
        write(layout, fLoteListSantander.Items[iLote].LISTSEGMENTOO.Items[iSeg].getLinhaLayout);

      inc(fLinhas, fLoteListSantander.Items[iLote].fLISTSEGMENTOA.Count);
      inc(fLinhas, fLoteListSantander.Items[iLote].fLISTSEGMENTOJ.Count);
      inc(fLinhas, fLoteListSantander.Items[iLote].fLISTSEGMENTOJ52.Count);
      inc(fLinhas, fLoteListSantander.Items[iLote].fLISTSEGMENTON.Count);
      inc(fLinhas, fLoteListSantander.Items[iLote].fLISTSEGMENTOO.Count);

      write(layout, fLoteListSantander.Items[iLote].getLinhaTrailer);
      inc(fLinhas);
    end;
...
function TArquivoSantander.getLinhaLayout: String;
var
  i : integer;
  ins_verifica : String;
...
begin
  ...
  ins_verifica := '';
  for i:= 0 to Self.LOTELISTSANTANDER.Count - 1 do
  begin
    if (ins_verifica <> '')and(ins_verifica <> self.LOTELISTSANTANDER.Items[i].INSCRICAO) then
    begin
    ...
    begin
      inscricao_util := self.LOTELISTSANTANDER.Items[i].INSCRICAO;
      tipo_util := self.LOTELISTSANTANDER.Items[i].TP_INSCRICAO;
      agencia_util := self.LOTELISTSANTANDER.Items[i].AGENCIA;
      conta_util := self.LOTELISTSANTANDER.Items[i].CONTA;
      dac_util := self.LOTELISTSANTANDER.Items[i].DAC_AGENCIA_CONTA;
      empresa_util := self.LOTELISTSANTANDER.Items[i].NOME_EMPRESA;
    end;
    ins_verifica := self.LOTELISTSANTANDER.Items[i].INSCRICAO;
  end;

  Result := LayoutToStr(lySantander) + '00000         ' +
      ...
      formataAlfa('Banco Santander', 30) +                //103
      DupeString(' ', 10) +                               //133
...
function TArquivoSantander.getLinhaTrailer: String;
begin
  result :=  LayoutToStr(lySantander) + '99999' +
             DupeString(' ',9) +                          //9
             formataNumero(fLoteListSantander.Count, 6) + //18
             formataNumero(fLinhas+1, 6) +                //24
             DupeString(' ', 211);                        //30
end;

function TArquivoSantander.getLote(codigo : integer): TLoteSantander;
var
  iLote : integer;
begin
  for iLote := 0 to fLoteListSantander.Count - 1 do
  begin
    if fLoteListSantander.GetItem(iLote).CODIGO = codigo then
    begin
      result := fLoteListSantander.GetItem(iLote);
      exit;
    end
  end;
  result := nil;
...
function TArquivoSantander.getRetorno(pos : integer): TPagamentoAlPagtoE;
begin
  Result := fRetorno.Items[pos];
end;

function TArquivoSantander.OcorrenciasToResultado(Ocorrencias: TAResultadoAlPagto): integer;
var
...
function TArquivoSantander.OcorrenciaStringToResultado(pSigla: String): TResultadoALPagto;
var
  i : integer;
  r : TResultadoALPagto;

begin
  for i := 0 to High(TOcorrenciaSantanderString) do
  begin
    if pSigla = TOcorrenciaSantanderString[i,0] then
    begin
       r.codigo := pSigla;
       r.descricao := TOcorrenciaSantanderString[i,1];
       r.resultado := TOcorrenciaSantanderString[i,2] = '0';
       r.tagERP := TOcorrenciaSantanderString[i, 3];
       Result := r;
       exit;
    end
  end;
...
procedure TArquivoSantander.popularRetorno;
var
  i, j : integer;
begin
  for i := 0 to fLoteListSantander.Count - 1 do
  begin
    for j := 0 to fLoteListSantander.Items[i].fLISTSEGMENTOA.Count - 1 do
      fRetorno.Add(segmentoAToPagamento(fLoteListSantander.Items[i].fLISTSEGMENTOA.Items[j]));
    for j := 0 to fLoteListSantander.Items[i].fLISTSEGMENTOJ.Count - 1 do
      fRetorno.Add(segmentoJToPagamento(fLoteListSantander.Items[i].fLISTSEGMENTOJ.Items[j]));
    for j := 0 to fLoteListSantander.Items[i].fLISTSEGMENTON.Count - 1 do
      fRetorno.Add(segmentoNToPagamento(fLoteListSantander.Items[i].fLISTSEGMENTON.Items[j]));
    for j := 0 to fLoteListSantander.Items[i].fLISTSEGMENTOO.Count - 1 do
      fRetorno.Add(segmentoOToPagamento(fLoteListSantander.Items[i].fLISTSEGMENTOO.Items[j]));
  end;
end;

function TArquivoSantander.retornaLote(pagamento: TPagamentoAlPagtoE): TLoteSantander;
var
  iLote, n : integer;
begin
  ...
  for iLote := 0 to fLoteListSantander.Count - 1 do
  begin
    if (fLoteListSantander.Items[iLote].TP_PAGAMENTO = pagamento.TP_PAGAMENTO) and
       (fLoteListSantander.Items[iLote].fFORMA_PAGAMENTO = pagamento.FORMA_PAGAMENTO) and
       (fLoteListSantander.Items[iLote].INSCRICAO = pagamento.INSCRICAO) and
       (fLoteListSantander.Items[iLote].AGENCIA = pagamento.AGENCIA) and
       (fLoteListSantander.Items[iLote].CONTA = pagamento.CONTA) and
       (fLoteListSantander.Items[iLote].DAC_AGENCIA_CONTA = pagamento.DAC_AGENCIA_CONTA) and
       (fLoteListSantander.Items[iLote].FINALIDADE_LOTE_PREFIXO = pagamento.FINALIDADE_LOTE_PREFIXO) and
       (fLoteListSantander.Items[iLote].FINALIDADE_LOTE = pagamento.FINALIDADE_LOTE) and
       (fLoteListSantander.Items[iLote].HISTORICOC_CC = pagamento.HISTORICOC_CC) then
    begin
      result := fLoteListSantander.Items[iLote];
      exit;
    end
  end;
  n := fLoteListSantander.Count + 1;
  with fLoteListSantander.New(self) do
  begin
    fCODIGO := n;
    ...
    result := fLoteListSantander.Items[fLoteListSantander.count - 1];
  end;
end;

function TArquivoSantander.segmentoAToPagamento(segmento: TSegmentoA): TPagamentoAlPagtoE;
var
  pagamento : TPagamentoAlPagtoE;

begin
  pagamento := TPagamentoAlPagtoE.Create;
  atualizarDadosLote(pagamento, segmento.fLoteSantander);
  ...
  if (segmento.LOTESANTANDER.FORMA_PAGAMENTO in [fpCreditoContaCorrenteBanco, fpChequePagamento, fpDocC,
      fpCreditoContaPoupancaBanco, fpCreditoContaCorrenteTitular, fpDocD, fpTedOutroTitular,
      fpTedMesmoTitular]) then
  begin
    ...
    if segmento.LOTESANTANDER.FORMA_PAGAMENTO = fpDocC then
      pagamento.FINALIDADE_DOC := segmento.FINALIDADE_DOC
    else
      if (segmento.LOTESANTANDER.TP_PAGAMENTO = tpSalarios) and (segmento.LOTESANTANDER.FORMA_PAGAMENTO = fpCreditoContaCorrenteBanco) then
        pagamento.STATUS_FUNCIONARIO := segmento.STATUS_FUNCIONARIO;
    pagamento.FINALIDADE_TED := segmento.FINALIDADE_TED;
  end
  else
    if (segmento.LOTESANTANDER.FORMA_PAGAMENTO = fpNotaFiscal) then
    begin
...
function TArquivoSantander.segmentoJToPagamento(segmento: TSegmentoJ): TPagamentoAlPagtoE;
var
  pagamento : TPagamentoAlPagtoE;
begin
  pagamento := TPagamentoAlPagtoE.Create;
...
function TArquivoSantander.segmentoNToPagamento(segmento: TSegmentoN): TPagamentoAlPagtoE;
var
  pagamento : TPagamentoAlPagtoE;
...
function TArquivoSantander.segmentoOToPagamento(segmento: TSegmentoO): TPagamentoAlPagtoE;
var
  pagamento : TPagamentoAlPagtoE;
begin
  pagamento := TPagamentoAlPagtoE.Create;
...
function TArquivoSantander.StrToAResultado(Ocorrencias: String): TAResultadoAlPagto;
var
  i : integer;
...
function TArquivoSantander.StrToTipoMoeda(var ok: boolean; const s: string): TTipoMoedaAlPagtoE;
begin
  result := StrToEnumerado(ok, s, ['BRL','009'], [tmReal, tmOutros]);
end;

function TArquivoSantander.TipoMoedaToStr(const t: TTipoMoedaAlPagtoE): string;
begin
  result := EnumeradoToStr(t, ['BRL','009'], [tmReal, tmOutros]);
end;

procedure TArquivoSantander.validaTipoForm(tipo: TTipoPagALPagtoE;
...
function TSegmentoJList.New(AOwner: TLoteSantander): TSegmentoJ;
begin
...
constructor TSegmentoJ.Create(AOwner: TLoteSantander);
begin
  self.fLOTESANTANDER := AOwner;
end;
...
constructor TSegmentoJ52.Create(AOwner: TLoteSantander);
begin
  self.fLOTESANTANDER := AOwner;
end;
...
begin
  Result := LayoutToStr(lySantander) +
    formataNumero(fLOTESANTANDER.fCODIGO, 4) +                     // 04
    '3' +                                                          // 08
...
function TSegmentoJ52List.New(AOwner: TLoteSantander): TSegmentoJ52;
begin
...
constructor TSegmentoN.Create(AOwner: TLoteSantander);
begin
  self.fLOTESANTANDER := AOwner;
end;
...
begin
  Result := LayoutToStr(lySantander) +
            ...
            formataAlfa(fLOTESANTANDER.NOME_EMPRESA, 30) +             //58
...
function TSegmentoNList.New(AOwner: TLoteSantander): TSegmentoN;
begin
...
constructor TSegmentoO.Create(AOwner: TLoteSantander);
begin
  fLOTESANTANDER := AOwner;
end;
...
function TSegmentoOList.New(AOwner: TLoteSantander): TSegmentoO;
begin
